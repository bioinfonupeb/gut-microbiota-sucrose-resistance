...
do
	echo "Processing ${i}";
	EXPID="${i}-trim"
	EXPDIR="experiments/${EXPID}"
#	rm -Rf "experiments/${i}"

	STEP="step-lefse-analysis"
	rm "nb-templates/${STEP}.ipynb"
    FILEPATH="${EXPDIR}/nb-executed-steps/${STEP}-${EXPID}.ipynb"
    echo "Removing file: ${FILEPATH}"
	rm ${FILEPATH}


#	./pipeline.sh "params/edited/${EXPID}.yaml" qiime2-2023.7
	./pipeline.sh "params/edited/${EXPID}.yaml" qiime2-amplicon-2024.5
#	./pipeline.sh "params/${i}.yaml" qiime2-2021.11 # QIIME2 version compatible with picrust2 plugin
